...
**Status — Part 2 (`AK40-10-issac-sim-emulator`, in `fsc_autopilot_ws/src/`, separate repo) built, SPEED mode verified working, EXTERNAL mode implemented but NOT yet verified:**
...
- Full design rationale, the sign-convention table, and all current parameter defaults are documented in the emulator's own `CLAUDE.md` (`fsc_autopilot_ws/src/AK40-10-issac-sim-emulator/CLAUDE.md`) — read that before touching SPEED-mode gains or units.
- **EXTERNAL mode is implemented (state machine ported, `force = ext_torque_cmd/effective_radius`, no feedforward — external controllers are expected to supply their own, matching `cable_torque_ctrl_node`'s existing BLSMC gravity term) but has not yet been runtime-verified.** This is the next session's starting point.

**Next steps (in order — see the plan file for full detail):**
1. Do the Part 1 cleanup above (debug print removal, joint limit revisit).
2. Verify EXTERNAL mode end-to-end: state transitions (`off→standby→running` via `~/enable_external_mode` service + `~/ext_torque_enable`), sign convention (`ext_torque_cmd` positive = retract, same as SPEED), authority hierarchy (ground station's `~/ext_mode_cmd="off"` always revokes access), and the `running→standby` fallback (should now hold position cleanly, same fix as SPEED's stale-command case — worth double-checking it actually does, since that fallback sets `control_mode_=SPEED` with `current_cmd_velocity_=0` and relies on the same PD+feedforward hold path).
3. Validate requirement #5 (`ros2 launch ak_motor_driver cable_torque_ctrl.launch.py`, unmodified, controls the sim cable in external mode via its own BLSMC/L1/UDE controller) and #6 (the `ak_motor_ground_station` GUI's Stop/Emergency-Stop/Exit-External interrupts correctly override the emulator).
