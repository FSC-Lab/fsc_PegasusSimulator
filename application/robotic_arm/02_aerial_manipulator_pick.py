...
TASK (default MODE = "pickplace_home"): a full ground-to-ground mission along
WORLD +y, the direction the arm faces —
  ...
  pick      descend alongside a tall block, close the jaws, climb out
...
The arm has exactly TWO setpoints (home and grasp) and the schedule min-jerks
between them during the transit flights — nothing is planned, and the arm only
moves while the vehicle translates, never while it is in contact with the block.
The DRONE does the aiming; the grip is a constant closing torque on the jaws.
Real contact plus a 0.2 kg unmodelled payload — the disturbance the GMO rejects.
...
Run with:  scripts/start_aerial_manipulator_pick.sh <config_name>
...
#   log_path (str)          npz destination (default log/pick_log.npz)
...
from fsc_aerial_manipulation.robotic_arm.utils_controller import control_params as CP   # gains: robotic_arm/config/pick.yaml
...
#   "pickplace" — full mission, the ARM REACHES down to grasp (moving-arm).
#   "pickplace_static" — the ARM DOES NOT MOVE (every joint stays at 0): the
#              DRONE flies over each point and descends straight down to
#              grasp/release. Stays airborne at the end (no landing).
#   "pickplace_home" — DEFAULT. The same mission, but it STARTS AND ENDS ON THE
#              GROUND and the arm has TWO setpoints: a folded HOME pose
#              ([0,40,40,0] deg) for takeoff and landing, and the straight-down
#              GRASP pose (q=0) for the pick/place. The swap happens DURING the
#              two transit flights, so the arm moves while the vehicle
#              translates. Nothing is planned — they are step setpoints the
#              schedule min-jerks between. The home pose is REQUIRED to land:
#              at q=0 the finger tips reach 51 mm BELOW ground at the resting
#              body height. See TRAJ_CONFIG and the LAND_* block below.
# Override per-run without editing this file: AM_SWEEP='{"traj_type":"circle"}'.
MODE = "pickplace_home"  # "hover"|"circle"|"circle_bent"|"pickplace"|"pickplace_static"|"pickplace_home"

# Start PAUSED (rendered runs only): the scene is fully built — vehicle seated
# on the ground, arm at the folded home pose (Q_SPAWN), tables and block spawned
# — and physics is frozen until PLAY is pressed in the GUI, so the initial
# geometry can be inspected first (this is the demo where a millimetre of prop
# placement decides whether the grasp works). One silent physics step runs
# before the pause (see run()): the arm pose and ground-seat live in the physics
# state and would otherwise not reach the renderer — the paused view would show
# the authored midair, q=0 asset instead of the true initial state.
...
# the body — and the resting height is known for the BODY. The pick props are
# placed from SPAWN_POS's xy and TAKEOFF_ALTITUDE only, so a purely vertical
# seat leaves the whole grasp geometry untouched.
...
#     coupled law is only proven AIRBORNE — ground contact is not in the model —
#     and "pickplace_home" both starts and ends on the ground, so the hold is
#     what keeps the arm folded and clear of the floor through the climb.
...
#     for the airborne-only modes ("pickplace_static", hover, circle).
...
# 14.6 mm at 0° and 41.9 mm at the +50° open stop (see PICKPLACE_PROPS below).
...
# Spawned automatically when TRAJ_TYPE is a pickplace mode: a TABLE + graspable
# block at the PICK grasp point, and an empty TABLE at the PLACE point. Grasp
# positions come from the same FK the trajectory uses, plus the measured pinch
# offset below. The block's world position is recorded in the npz ("obj_p") —
# the carry is measurable, not just visual.
PICKPLACE_PROPS = True

# ── MEASURED gripper geometry (headless gripper_joint sweep at arm q=0; every
# offset is in the EE frame, +z up the tool axis, x = the jaw closing axis) ────
# The fingers are L-shaped. The jaw is NARROWEST in a band 0.055–0.065 m below
# the EE (the PINCH BAND — this is what actually grips), and the finger TIPS
# reach 0.124 m below the EE, i.e. 0.064 m BELOW the pinch band. That 64 mm
# overhang is the whole reason the object has to be tall and gripped near its
# top: grip an object at mid-height and the tips go through the table.
# Jaw gap at the pinch band vs gripper_joint angle (measured, ~0.55 mm/deg):
#     +50° 41.9 mm   +30° 31.0   +15° 22.7   0° 14.6   −25° 3.0   −50° closed
# The jaws are EFFORT-driven (see _gripper_effort), so "open" is the +50°
# authored limit the constant opening effort parks them against → 41.9 mm.
# (The old "pad gap ~7 cm open / ~3.5 cm closed" comment measured the grip-link
# ORIGIN separation, not the jaw opening — the real jaw is less than half that.
# GRIPPER_OPEN/GRIPPER_CLOSED/GRIPPER_RATE below are dead constants from the
# position-servo era and do not affect the grip.)
GRASP_OFF_EE   = np.array([0.0, 0.0, -0.060])   # pinch-band centre, EE frame [m]
FINGER_TIP_OFF = -0.1239                        # lowest finger point, EE frame [m]
# CEILING — the hard limit on how far the object may stick up between the jaws.
# Directly above the pinch band sits the gripper body, and it is WIDE:
#   hub          z ∈ [−0.0579, −0.0393], x ±0.011, y ±0.020   (EE frame)
#   manip_base   z ∈ [−0.0457, +0.0025], x ±0.064, y ±0.038   <- a 128 mm plate
# so the lowest fixed structure is the hub underside at EE−0.0579 — only 2 mm
# above the pinch band. Anything the jaws are meant to grip MUST stay below it.
OBJ_TOP_OFF_EE = -0.062     # [m] where the block's TOP face is placed, EE frame

# Payload: a TALL BLOCK, gripped near its top and deliberately NOT enveloped by
# the jaws (the fingers straddle its upper part; everything below the pinch band
# just hangs alongside). x is the jaw axis and stays at the 0.03 m that already
# flew — at the 41.9 mm open gap that is 5.9 mm of approach clearance per side,
# against a measured EE placement error of ~4 mm. y is free (the pads are ~58 mm
# wide) so the block is widened there for standing stability.
OBJ_XYZ  = np.array([0.03, 0.06, 0.10])   # [m] (x = jaw axis, y, z = height)
OBJ_MASS = 0.20                            # [kg] deliberately unmodeled payload
# HEIGHT is set by the finger tips, TOP by the ceiling above (OBJ_TOP_OFF_EE):
#   tip clearance = OBJ_XYZ[2] − |FINGER_TIP_OFF − OBJ_TOP_OFF_EE| = 0.100 − 0.062
#                 = 0.038 m of table clearance / vertical over-descent budget.
# The jaws grip the block's TOP ~20 mm of side face and nothing else — the
# fingers reach 62 mm further down and just hang alongside it. This is what
# "grasp it but not entirely" has to mean on this gripper.
#
# HOW THIS WAS SIZED — two headless runs, both of which pushed the block up into
# the gripper body, are why the ceiling above is a hard constraint:
#   run 1  block 0.14 m, top at EE−0.020 (into the manip_base plate): the closing
#          jaws rammed the plate, the arm folded to q2 ≈ +27° and rode the jaws
#          +40 mm UP, so the grip ended up on the block's top EDGE. It completed
#          the mission, but on luck, not geometry.
#   run 2  block 0.155 m, top at EE+0.010 (10 mm ABOVE the EE origin): same
#          mechanism, much harder — q2 saturated at its +50° limit, the arm
#          torque sat on the 1.5 N·m clamp, and the vehicle crashed at t = 29.8 s
#          just after release.
# In BOTH runs the EE was exactly on its reference at t = 14 s (descent complete,
# jaws still open) and only moved when the grip closed — so this is contact with
# the gripper's own body, NOT a tracking or trajectory problem. Keeping the
# block's top below the hub underside removes the contact entirely.

# Tables, replacing the old thin pedestals (2 cm pick / 5 cm place). Those had
# to stay thinner than the object because the jaws closed at the object's
# mid-height, level with the support — a wide top would have been grabbed
# instead of the object. Gripping near the top of a tall object lifts the whole
# jaw clear of the surface, so the support can now be any size, and both ends of
# the mission get the same table.
TABLE_XY        = 0.30    # [m] square table top
...
# Measured clearance check at the grasp hover (CoM 1.5 m, arm q=0): EE 1.308,
# pinch band 1.248, finger tips 1.184, table top 1.148, and the landing legs sit
# at 1.235 — 51 mm ABOVE the finger tips, so the tips (not the legs) are the
# lowest part of the vehicle and set the table clearance.
...
# ── Control parameters: robotic_arm/config/pick.yaml ─────────────────────
...
class InProcessDemoV2:
    ...
    def __init__(self):
        ...
        # "pickplace"/"pickplace_static") spawn the arm hanging at q = 0, whose
        # finger tips reach 0.356 m below the base — 51 mm THROUGH the floor at
        # the resting height. Those modes neither start nor end on the ground,
        # so they keep the authored midair SPAWN_POS and simply fly from there.
        if LAND_ENABLE:
        ...
        self._obj_p0 = None                      # block position at t=0 (carry distance)
    ...
    def _panel(self, p0, R0, q, res, tau_j, ref, x_c):
        ...
        rows = [
            ...
            # the grip is a constant TORQUE, so the commanded effort and the
            # angle it actually reaches are different facts — both are shown
            # (a jaw stalled on the block sits far from the closed limit).
        ...
        # the block: where it is now, and how far it has been moved from where
        # it spawned — the one number that says whether the mission worked.
        if self._obj is not None and self._obj_p0 is not None:
            try:
                pos, _ = self._obj.get_world_pose()
                d = float(np.linalg.norm(np.asarray(pos, float) - self._obj_p0))
                rows.append(_row("BLOCK", "pos", "[m]", _vec(pos, "%+8.3f")))
                rows.append(_row("", "moved", "[m]", f"{d:8.3f}"))
            except Exception:
                pass
    ...
    def _save_history(self):
        ...
        # pick_log.npz, NOT gmo_log.npz — 02_aerial_manipulator_free.py writes
        # that one, and both demos share this LOG_DIR (one npz per rig, see
        # plot_results.py). A shared name means whichever rig ran last silently
        # overwrites the other's data, and plot_results' "newest log" default
        # then plots a run from the wrong rig.
        path = SWEEP.get("log_path") or os.path.join(LOG_DIR, "pick_log.npz")
    ...
    def _spawn_table(self, tag, xy, top_z):
        """A table (top slab + 4 legs) whose TOP SURFACE sits at top_z."""
        from omni.isaac.core.objects import FixedCuboid
        FixedCuboid(prim_path=f"/World/{tag}_table_top", name=f"{tag}_table_top",
                    position=np.array([xy[0], xy[1], top_z - TABLE_TOP_THK / 2.0]),
                    scale=np.array([TABLE_XY, TABLE_XY, TABLE_TOP_THK]),
                    color=np.array([0.55, 0.42, 0.28]))
        leg_h = max(top_z - TABLE_TOP_THK, 1e-3)          # slab underside -> floor
        d = TABLE_XY / 2.0 - TABLE_LEG_INSET - TABLE_LEG / 2.0
        for i, (sx, sy) in enumerate(((+1, +1), (+1, -1), (-1, +1), (-1, -1))):
            FixedCuboid(prim_path=f"/World/{tag}_table_leg{i}",
                        name=f"{tag}_table_leg{i}",
                        position=np.array([xy[0] + sx * d, xy[1] + sy * d, leg_h / 2.0]),
                        scale=np.array([TABLE_LEG, TABLE_LEG, leg_h]),
                        color=np.array([0.30, 0.25, 0.20]))

    def _spawn_pickplace_props(self):
        """Table + graspable block at the pick point, empty table at the place
        ...
        if not PICKPLACE_PROPS or TRAJ_TYPE not in ("pickplace", "pickplace_static",
                                                      "pickplace_home"):
            return
        try:
            from omni.isaac.core.objects import DynamicCuboid
            cfg = P.TRAJ_CONFIG[TRAJ_TYPE]
            # the pose the arm is in AT THE GRASP: pickplace_home has a separate
            # q_grasp (it is folded at q_home for takeoff/landing), moving-arm
            # pickplace reaches to q_target, static holds q_hold throughout.
            qT = np.array(cfg.get("q_grasp",
                                  cfg.get("q_target", cfg.get("q_hold"))), float)
            # anchor xy comes from the CoM at the SPAWN pose (the arm may be
            # folded there, which moves the CoM relative to the body)
            _, r_c0, _, _ = P.arm_fk(Q_SPAWN)
            r_eT, r_cT, _, _ = P.arm_fk(qT)
            R_e = np.eye(3)
            ...
            anchor = np.array([SPAWN_POS[0] + r_c0[0], SPAWN_POS[1] + r_c0[1],
                               P.TAKEOFF_ALTITUDE])
            # ee[tag] = where the EE ORIGIN ends up when the vehicle hovers over
            # that waypoint; every prop height is then an EE-frame offset from it
            # (grasp band, hub ceiling, finger tips are all measured that way).
            ee, grasp = {}, {}
            for tag in ("pick", "place"):
                wp = np.array(cfg[f"{tag}_wp"], float)
                ee[tag] = anchor + wp + (r_eT - r_cT)
                grasp[tag] = ee[tag] + R_e @ GRASP_OFF_EE
            # block top goes just under the gripper-body ceiling; the table it
            # stands on is one block-height below that.
            top_z = float((ee["pick"] + R_e @ np.array([0.0, 0.0, OBJ_TOP_OFF_EE]))[2])
            table_z = top_z - float(OBJ_XYZ[2])
            for tag in ("pick", "place"):
                self._spawn_table(tag, ee[tag][:2], table_z)
            self._obj = DynamicCuboid(
                prim_path="/World/pick_object", name="pick_object",
                position=np.array([grasp["pick"][0], grasp["pick"][1],
                                   top_z - OBJ_XYZ[2] / 2.0]),
                scale=OBJ_XYZ.copy(),
                color=np.array([0.9, 0.15, 0.15]), mass=OBJ_MASS)
            tip_clear = (ee["pick"][2] + FINGER_TIP_OFF) - table_z
            print(f"[InProc] props: table tops z={table_z:.3f} m "
                  f"({TABLE_XY:.2f}x{TABLE_XY:.2f} m), block {OBJ_XYZ.round(3)} m "
                  f"{OBJ_MASS:.2f} kg, top z={top_z:.3f} m "
                  f"({(top_z - grasp['pick'][2])*1000:+.0f} mm vs the pinch band, "
                  f"must stay below +2), finger tips clear the table by "
                  f"{tip_clear*1000:.0f} mm", flush=True)
            try:
            ...
            print(f"[InProc] pickplace grasp points: pick {np.round(grasp['pick'], 3)}, "
                  f"place {np.round(grasp['place'], 3)}", flush=True)
        except Exception as exc:
            self._obj = None
    ...
    def _gripper_effort(self, grip_closed):
        ...
        (closing) squeeze holds the cube; positive spreads the jaws. Stores the effort
    ...
    def _control_step_inner(self, dt):
        ...
        if self._phase == "takeoff":
            ...
            if dwell >= HOVER_DWELL:
                ...
                # Prop placement is UNAFFECTED: the props were spawned from the
                # same nominal anchor, and the re-anchor only absorbs the ~0.1 m
                # hover offset the vehicle is actually sitting at.
        ...
        # never pre-compensates a reaction the arm is not producing.
        ...
        # phases — the block is on its table at takeoff and already released
        # before the plan's last leg.
        ...
        # rejects the payload/grip contact. The software PD hold owns the arm
