#!/usr/bin/env python
"""
| File: 04_px4_multi_drone_rigidbody_payload.py
...
class FscDroneSim:
    ...
    def __init__(self,
                 ...
                 payload_dimension: tuple=(0.2, 0.3, 0.05),
                 payload_hook_local_ratio: tuple=((0.5, 0.0, 0.5), 
                                                  (-0.5, 0.5, 0.5), 
                                                  (-0.5, -0.5, 0.5)),
        ...
        self.payload_dimension = payload_dimension
        self.payload_hook_local = tuple(
            tuple(dim * r for dim, r in zip(payload_dimension, ratio))
            for ratio in payload_hook_local_ratio)
        ...
        self.payload_init_pos, self.uav_init_positions = sl.setup_payload_and_triangle_uavs_rigid(
            L=self.cable_length,
            z=self.z_uavs,
            center_xy=self.center_xy,
            p_hook = self.payload_hook_local
        )
        ...
        self.payload_prim = sl.create_brick_with_xform_root(
            ...
            length_x=self.payload_dimension[0],
            width_y=self.payload_dimension[1],
            height_z=self.payload_dimension[2],         
            mass=payload_mass,
        ...
        for i in range(self.num_of_drones):
            payload_hook_pos = [p + h for p, h in zip(self.payload_init_pos, self.payload_hook_local[i])]
            mid = sl.get_mid_point(payload_hook_pos, self.uav_init_positions[i])
        ...
        self.rigid_body_paths = [payload_path]
        config_ros2_rigid_body = {
            "pub_state": True,
            "sub_force": True,
            "topic_prefixes": ["payload"]
        }
        ...
        for i in range(self.num_of_drones):
            ...
            con.create_spherical_joint(
                    ...
                    local_pos1=self.payload_hook_local[i], # cable connected to the center of the payload
            )
