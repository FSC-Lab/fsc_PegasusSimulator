...
PEGASUS_SCRIPT="$FSC_PEGASUS_ROOT/application/px4_base/01_px4_single_drone.py"
PX4_BUILD_DIR="$PX4_DIR/build/px4_sitl_default"
PX4_BIN="$PX4_BUILD_DIR/bin/px4"
PX4_ROOTFS_BASE="$PX4_BUILD_DIR/rootfs"
# This directory owns the indoor parameters.bson and is never shared with the
# outdoor launchers. Keep PX4 instance 0 so Pegasus still uses TCP port 4560,
# MAVLink system ID 1, and ROS namespace /uav_0.
PX4_WORK_DIR="$PX4_BUILD_DIR/rootfs_fsc_indoor"
SESSION="px4_isaac"
DELAY=2
...
GROUNDTRUTH_LOG="/tmp/indoor_iris_groundtruth.log"
...
if [[ ! -d "$PX4_WORK_DIR" ]]; then
  echo "Initializing persistent indoor PX4 profile: $PX4_WORK_DIR"
  cp -a "$PX4_ROOTFS_BASE" "$PX4_WORK_DIR"
fi

tmux has-session -t "$SESSION" 2>/dev/null && tmux kill-session -t "$SESSION"

tmux new-session -d -s "$SESSION" -n "indoor_iris" "
cd \"$PX4_DIR\" || { echo 'PX4_DIR not found'; exec bash; }
echo 'Starting indoor Iris PX4 SITL with persistent OptiTrack parameters...'
...
# The PX4_PARAM_* overrides are consumed before EKF2 starts, including the
# reboot-required height, magnetometer, and delay settings. Save and print the
# effective profile after boot so every run leaves an auditable parameter block
# in the PX4 pane and persists it only in rootfs_fsc_indoor/parameters.bson.
(
  sleep 4
  for parameter in \
    EKF2_HGT_REF EKF2_MAG_TYPE EKF2_GPS_CTRL EKF2_EV_CTRL EKF2_EV_DELAY COM_ARM_WO_GPS; do
...
sleep $DELAY
echo 'Launching indoor Iris in Isaac Sim...'
